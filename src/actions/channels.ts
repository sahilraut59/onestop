...
export async function createChannel({ name, userId, workspaceId }: { workspaceId: string; name: string; userId: string }) {
    ...
    const [, updateChannelMemberError] = await updateChannelMembers(channelRecord[0].id, userId);

    if (updateChannelMemberError) {
        return { error: 'Update Members Channel Error' }
    }

    // Add channels to user's channel array
...
const addChannelToUser = async (userId: string, channelId: string) => {
    ...
    const { data: addChannelData, error: addChannelError } =
        await supabase.rpc("update_user_channels", { user_id: userId, channel_id: channelId });
...
export const updateChannelMembers = async (channelId: string, userId: string) => {
    ...
    const { data: updateChannelData, error: updateChannelError } =
        await supabase.rpc('update_channel_members', { new_member: userId, channel_id: channelId });

    return [updateChannelData, updateChannelError];
}

export const updateWorkspaceChannel = async (channelId: string, workspaceId: string) => {
    const supabase = await supabaseServerClient();

    const { data: updateWorkspaceData, error: updateWorkspaceError } =
        await supabase.rpc('add_channel_to_workspace', { channel_id: channelId, workspace_id: workspaceId });
